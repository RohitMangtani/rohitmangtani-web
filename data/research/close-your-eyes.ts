...
export const closeYourEyesData: ResearchData = {
  ...
  sections: [
    ...
    },
    {
      ...
      content: `
        <p>You are building a fraud detection system. You have a thesis. Providers billing above a certain threshold in specific geographic clusters are more likely to be submitting anomalous claims. You believe this because you have seen the patterns in the data, over months of analysis, and you want to build a system that flags them automatically.</p>

        <p>So you code it. Or you describe it to an AI and it codes it. Either way, the system runs. A report comes back. Numbers on a screen. Precision: 78%. Recall: 64%. False positive rate: 12%. Is that good? You think so. But you cannot see what happened. You cannot see the moments where the system caught a real anomaly versus the moments where it flagged a legitimate high-volume provider. You cannot see whether the false positives came from a flaw in your thesis or from an edge case your logic did not handle. The numbers tell you the outcome. They do not show you the process.</p>

        <p>Now picture the same thing, but you can see it. Every flagged provider rendered on a map. Color-coded by confidence level. Real anomalies highlighted in green, ambiguous cases in amber, false positives in red. You look at the red flags and you see the pattern immediately. The system is flagging rural providers who serve large geographic areas. Their billing looks anomalous by volume, but their patient base is spread across three counties. Your thesis was right. Your filter was wrong. You say "adjust the threshold by service area radius." The system adjusts. You see the red flags disappear. The amber ones sharpen. The green ones hold.</p>

        <div class="p-5 border border-[var(--border)] rounded-lg bg-[var(--bg-secondary)] my-6">
          <p class="text-sm text-[var(--fg-muted)]">You did not need to understand the code. You did not need to read a statistical report. You saw the problem, because the system rendered it in a way your brain could process instantly. The 13-millisecond image processing that MIT measured is doing the work that hours of log analysis used to do. Your eyes found the pattern. Your judgment named it. Your words fixed it.</p>
        </div>
