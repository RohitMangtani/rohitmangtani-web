...
export const hiveData: ResearchData = {
  ...
  date: '2026',
  title: 'Hive',
  subtitle: 'A dashboard for running up to eight AI agents at the same time.',
  liveUrl: 'https://github.com/RohitMangtani/hive',

  onRamp: {
    description:
      'Up to eight AI agents running at once. One screen shows all of them. Green means working, red means done, yellow means stuck. Tap any tile to talk to that agent. Hive is what I built to keep track of everything when I started running multiple agents on different projects at the same time.',
    whatItDoes:
  ...
  claim:
    'I needed a way to see what all my agents were doing without reading terminals at once. So I built a dashboard that shows me.',
  ...
  sections: [
    {
      ...
      content: `
<p>You are lying on your bed. Your laptop has terminal windows arranged in a grid. Top-left is refactoring authentication. Top-right is writing content. Bottom-left is debugging a deploy. Bottom-right is investigating a production bug. You could have four agents running or eight. You pick up your phone and open the dashboard. The tiles match your terminal layout. Top-left tile maps to top-left terminal. Bottom-right tile maps to bottom-right terminal. Each tile has a dot. Green means that agent is working. Red means it is done. Yellow means it needs you.</p>

<p>You see a red dot in the top-right tile. You already know what that is without reading the label. It is the content agent, because that is where the content terminal sits on your screen. You tap it and type: "Start the next chapter." The dot turns green. You glance at the other tiles. All green. You put the phone down.</p>

<p>Twenty minutes later you check again. Two green, one red, one yellow. The yellow one is bottom-left. You know it is the deploy debugger because that is where you put it. You read its question, type a two-sentence answer, and the dot turns green. The red one finished its task. You type a new one. All your agents, all visible, all reachable, from your phone.</p>
...
<p>That is part of the point of the system. You do not have to pick one model and live inside its blind spots. You can let one model push something forward, let another audit it or build from it, and use the differences between them to catch corners a single model would miss. And if your preferred agent is not one of the three that ship built-in, you add it with a config file. Or you ask one of your running agents to add it for you. The dashboard is the layer above the models, not tied to any of them.</p>
    ...
    },
    {
      ...
      title: 'The Gap',
      content: `
<p>Open Claude Code right now. It works well. You give it a task, it reads your codebase, runs commands, writes code, iterates. Within a single terminal, the experience is strong and getting stronger every quarter. Now open a second instance. And a third. And a fourth. And keep going until you have eight.</p>
...
<p>Each tile shows green, red, or yellow. Getting the color right takes several overlapping signals: real-time hooks where they exist, pattern analysis of session log files, CPU-based idle detection, and terminal-output checks. The layers cross-validate each other. One method alone produces false positives. Together they produce a reliable signal.</p>
        ...
        },
        {
          title: 'Auto-Pilot',
          content: `
<p>Claude Code pauses for permission prompts. "Allow this bash command?" "Approve this file edit?" With one terminal, you click yes and move on. With several agents, these prompts stack up. One agent pauses, you do not notice for ten minutes, and an hour of potential output is lost.</p>
...
<p>Auto-pilot handles it. Routine prompts auto-approve within a fifteen-second grace window. The dashboard shows the prompt briefly so you can override if needed. If you do nothing, the agent continues. A watchdog also monitors for stuck loops where an agent keeps calling the same tool repeatedly, and flags it on the dashboard so you can step in.</p>
        ...
        },
        {
          title: 'Coordination',
          content: `
<p>Multiple agents working in shared codebases will eventually collide. Two editing the same file. One designing an API another needs to consume.</p>
...
<p>Hive's learning system prevents this. When an agent solves something non-obvious, it writes the lesson to a file in the project. Every future agent reads that file before starting. The lessons compound across sessions and across agents. After months of running, the system knows things about your projects that no fresh agent could replicate.</p>
    ...
    },
    {
      ...
      content: `
<p><strong>Talk to agents from anywhere.</strong> Tap any tile, type a message, and it goes straight to that agent's terminal. If the agent is busy, messages queue up and drain automatically when it is ready. The grid on your phone matches your screen, so you always know which agent you are talking to.</p>
...
<p><strong>Walk away.</strong> Start your agents, close your laptop, go do something else. Come back and the dashboard shows you what happened. Green tiles kept working. Yellow tiles are waiting. Red tiles finished. You pick up where things paused without re-reading anything.</p>
...
<p><strong>Push notifications.</strong> When an agent goes yellow, macOS sends a native notification with the project name and what it needs. You do not have to be looking at the dashboard to know something needs attention.</p>
    ...
    {
      id: 'four-contexts',
      title: 'Eight Contexts, Not Eight Workers',
      content: `
...
<p>Now watch someone run eight instances across different projects. One is an hour deep into an authentication refactor, with full context on every file it has touched. Another has been debugging a deployment pipeline for thirty minutes, with a mental model of the infrastructure built step by step. A third is writing content, tuned to the site's voice after reading the style guide and ten existing articles. A fourth is iterating on a video processing pipeline, aware of every edge case from previous runs. The other four are working on their own problems with their own accumulated context. It started with four. It grew to eight because the workflow kept scaling.</p>
...
<p>These two forces multiply. A smarter model reading a thicker knowledge base produces better output than either force alone. And both forces make the management layer more necessary, not less. The smarter the agents get, the more of them you run. The more personal context accumulates, the more valuable each session becomes. It started at four, went to eight, and will keep going as context windows and hardware allow it. The infrastructure that lets you manage the fleet is what makes scaling the fleet possible.</p>
      `.trim(),
    },
    {
      id: 'the-end-state',
      title: 'The End State',
      content: `
<p>Right now Hive runs up to eight terminal agents on one machine. I run a mix of Claude Code, Codex, and OpenClaw. Different providers, different agent architectures, same grid. It started as four. The workflow kept scaling, so the system scaled with it. That is the proof of concept. The end state is bigger than Claude, bigger than Anthropic, bigger than any single model provider.</p>
...
<p>That is where this goes. The proof of concept is up to eight terminal agents on a MacBook with a stoplight dashboard on your phone, today Claude, Codex, and OpenClaw running side by side. The product is the first car for AI labor. Provider-agnostic. A steering wheel for fleet operations. A speedometer that shows status as color, not text. Mirrors that give you spatial memory across every running agent. And eventually, a layer that turns natural language into fleet operations. The interface that lets any person, not just engineers, direct any combination of agents from any company through one screen. Green means driving. Red means stopped. Type into the tile. The agent moves.</p>
      `.trim(),
    },
    {
...
<p>If you want to try it the same way I use it, the default GitHub path is: install Claude Code and/or Codex (OpenClaw also works if you prefer), run setup, then run <code>npm start</code> and <code>npm run deploy:dashboard</code>. That gives you your own hosted Hive dashboard in a few minutes while the agent terminals still run locally on your machine. Start with one or two agents and scale up as you get comfortable. The dashboard adapts to however many you are running, up to eight.</p>
